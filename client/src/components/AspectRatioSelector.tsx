        ...
        <div className="space-y-3 dark">
            ...
            <div className="grid grid-cols-3 gap-2">
                {aspectRatios.map((ratio) => {
                    ...
                    return (
                        <button key={ratio} type="button" onClick={() => onChange(ratio)} className={`flex flex-col sm:flex-row items-center justify-center gap-2 rounded-md border px-2 sm:px-5 py-2.5 text-sm transition border-white/10 ${selected ? 'bg-white/10' : 'hover:bg-white/6'}`}>
                            {iconMap[ratio]}
